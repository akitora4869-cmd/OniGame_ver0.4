...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private UUID oni, testParticipant; private OniType oniType=OniType.DAKKO, selectedOniType; private int brokenHearts, totalHearts, secondsLeft; private BukkitTask ticker, sidebarTicker, chaseTicker, repairTicker, bgmTicker; private boolean finalPhase; private String activeBgmSound;
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] a){
        ...
        if(a[0].equalsIgnoreCase("start")){OniType type=a.length>1?OniType.parse(a[1]):null;start(type,sender);return true;}
        if(a[0].equalsIgnoreCase("stop")){end(false,"管理者が試合を終了しました");return true;}
        if(a[0].equalsIgnoreCase("status")){String mode=getConfig().getString("heart-placement-mode","random");msg(sender,"状態: "+state+" / 参加: "+participants.size()+" / 心臓: "+brokenHearts+"/"+totalHearts+" / 配置: "+mode+" / 選択鬼: "+(selectedOniType==null?"ランダム":selectedOniType.display));return true;}
    ...
    private void start(OniType forced,CommandSender starter){
        ...
        if(soloTest){if(!(starter instanceof Player gm)){msg(starter,"参加者0人のテスト開始はゲーム内のGMが実行してください。");return;}testParticipant=gm.getUniqueId();participants.add(testParticipant);msg(gm,"&e参加者0人のため、GMを鬼にしたテストモードで開始します。");}
        else if(participants.size()<getConfig().getInt("minimum-players",2)){all("参加人数が足りません。0人テストまたは必要人数以上で開始してください。");return;}
        ...
        resetRuntime(); List<UUID> ids=new ArrayList<>(participants); Collections.shuffle(ids); oni=ids.remove(0); players.addAll(ids); oniType=forced!=null?forced:(selectedOniType!=null?selectedOniType:OniType.values()[new Random().nextInt(2)]);
        ...
        for(Location l:hearts){l.getBlock().setType(heartMat);heartHp.put(LocationStore.encode(l),getConfig().getInt("heart-max-health",100));}
        totalHearts=heartHp.size();secondsLeft=getConfig().getInt("game-seconds",1200);state=GameState.RUNNING;
        Player op=Bukkit.getPlayer(oni); setupOni(op,os); for(UUID id:players)setupPlayer(Bukkit.getPlayer(id),ps);if(soloTest)giveTestPlayBook(op);
    ...
    private void resetRuntime(){stopFinalPhaseBgm();players.clear();escaped.clear();dead.clear();chased.clear();lastSeenAt.clear();heartbeatAt.clear();repairingHeart.clear();repairProgressAt.clear();nextSkillCheckAt.clear();skillChecks.clear();heartAlertAt.clear();cooldowns.clear();heartHp.clear();brokenHearts=0;finalPhase=false;if(ticker!=null)ticker.cancel();}
    ...
    private void updateHorrorState(){
        if(state!=GameState.RUNNING||oni==null)return;Player hunter=Bukkit.getPlayer(oni);if(hunter==null)return;long now=System.currentTimeMillis();
    ...
    private void notifyHeartAttack(Location heart,String heartKey,long now){long cooldown=(long)(getConfig().getDouble("heart-alert-cooldown-seconds",5.0)*1000L);if(now-heartAlertAt.getOrDefault(heartKey,0L)<cooldown)return;heartAlertAt.put(heartKey,now);Player hunter=Bukkit.getPlayer(oni);if(hunter==null)return;msg(hunter,"&4心臓が傷つけられている…… &c"+directionFrom(hunter.getLocation(),heart));hunter.playSound(hunter.getLocation(),Sound.ENTITY_WARDEN_HEARTBEAT,1.0f,0.55f);}
    private void forceHeartAlert(Location heart){Player hunter=Bukkit.getPlayer(oni);if(hunter==null)return;msg(hunter,"&4作業失敗の大きな音！ &c"+directionFrom(hunter.getLocation(),heart));hunter.playSound(hunter.getLocation(),Sound.ENTITY_GENERIC_EXPLODE,0.8f,0.7f);}
    ...
    @EventHandler public void onDamage(EntityDamageByEntityEvent e){if(state!=GameState.RUNNING||!(e.getEntity() instanceof Player victim))return;Player attacker=e.getDamager() instanceof Player p?p:(e.getDamager() instanceof Projectile pr&&pr.getShooter() instanceof Player p?p:null);if(attacker==null)return;boolean av=attacker.getUniqueId().equals(oni), vv=victim.getUniqueId().equals(oni);if(av==vv&&!getConfig().getBoolean("friendly-fire",false)){e.setCancelled(true);return;}if(repairingHeart.containsKey(victim.getUniqueId()))stopRepair(victim,"&c攻撃を受け、心臓への干渉が中断された。");if(vv&&brokenHearts<totalHearts)e.setDamage(e.getDamage()*.1);}
